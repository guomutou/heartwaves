        ...
        <div class="row wrapper border-bottom white-bg page-heading">
            ...
            <div class="row">
                ...
                <div class="ibox float-e-margins">
                    ...
                    <thead>
                    <form action="/heartwaves/index.php/Admin/Jingli/searchjingli" method="post" >
            <a href = "/heartwaves/index.php/Admin/Jingli/addjingli"><button type = "button"> 新增普通管理员</button></a>&nbsp;&nbsp;

             <input  id="sear" type = "text" name = "search" placeholder = "普通管理员搜索" onchange = "search()" >    <input type="submit"  value="搜索"><br><br>

            </form>
            <form action="/heartwaves/index.php/Admin/Jingli/editjingli" method="post" >
                    ...
                    <tr class="gradeU">
                       <!--&lt;!&ndash;  <td><?php echo ($vo["id"]); ?></td> &ndash;&gt;-->
                        <input type="hidden" name="id" value="<?php echo $v[id]?>"></td>
                        <input type="hidden" name="id" value="<?=$v[id]?>" >
                       <td class="catt"><input type="text" name="username" value="<?=$v['username']?>" attid="<?=$v['id']?>"></td>

                        <td><?php echo date('Y-m-d',$v[create_time])?></td>
                        <td><?php echo $v[kouling]?></td>
                        <td><?=$v['yonghu']?></td>
                        <td class="user_num"><input name="user_num" type="text" value="<?php echo $v['user_num']?>" attid="<?=$v['id']?>"></td>
                        <td  class="statuss" >

                            <select name = "status"  attid="<?=$v['id']?>" >
                            <?php if ($v['status']==0): ?>
                            <option selected  value="0">正常</option>

                            <option value="1">已经锁定</option>
                            <?php endif?>
                            <?php  if ($v['status']==1):?>
                            <option   value="0">正常</option>

                            <option selected  value="1 ">已经锁定</option>
                            <?php endif?>
                            </select>
                        </td>
                        <td class="endtime" >
                        ...
                        <div class="input-group input-append date form_date">
                            <input type="text" class="" readonly id="end_time" name="endtime" value="<?php echo date('Y-m-d H:i:s',$v[end_time]);?>"  placeholder="" attid="<?=$v['id']?>">
                            <span class="input-group-addon" id="basic-addon2"><span class="glyphicon glyphicon-time" aria-hidden="true"></span></span>
                        </div>
                        <!-- <input  name="endtime" class="sang_Calender" type="text" value="<?=date('Y-m-d',$v['end_time'])?>" attid="<?=$v['id']?>"> -->

                        </td>

                        <td style = "width:500px;">
                        <!-- <button type="submit" class="btn btn-w-m btn-info">修改</button></a> -->
                            <a href="<?=U('Organizeconstruct/getuser?id='.$v['id'])?>" target="_blank"><button type="button" class="btn btn-w-m btn-danger">查看用户</button></a>
                            <a href="/heartwaves/index.php/Admin/Jingli/deleteorg/id/<?php echo $v[id]?>"  onclick= "if(confirm( '如果删除该组，组内的成员将一起被删除 ')==false)return   false; " target="_blank"><button type="button" class="btn btn-w-m btn-danger">删除</button></a>
...
<script>
    ...
    <script type="text/javascript" src="/heartwaves/Public/js/datetime.js"></script>
    <!-- 时间插件 -->
<script type="text/javascript" src="/heartwaves/Public/js/plugins/datepicker/bootstrap-datetimepicker.js" charset="UTF-8"></script>
<script type="text/javascript" src="/heartwaves/Public/js/plugins/datepicker/locales/bootstrap-datetimepicker.fr.js" charset="UTF-8"></script>
<script type="text/javascript" src="/heartwaves/Public/js/plugins/datepicker/locales/bootstrap-datetimepicker.zh-CN.js" charset="UTF-8"></script>
...
//            window.location = "http://localhost/heartwaves/index.php?m=Admin&c=Jingli&a=searchjingli&str="+x.value;
//                console.log(x);
    }
</script>
	<script type="text/javascript">
    // 修改普通管理员姓名  
          $('.catt input').blur(function(){
            //获取值
           ...
           //发送post请求
            $.post("<?php echo U('Jingli/editjingliname');?>",PostData,
                function(data){
                });

        })
    // 修改普通管理员到期时间  
        $(".endtime input").change(function(){
            var id= $(this).attr("attid");
                var va= $(this).val();
                var PostData= {
                    'id':id,
           ...
           //发送post请求
            $.post("<?php echo U('Jingli/endtime');?>",PostData,
                function(data){
                });
        });
    // 修改普通管理员状态  
        $('.statuss select').blur(function(){
            //获取值
                var id= $(this).attr("attid");
                var va= $(this).val();
                var PostData= {
           ...
           //发送post请求
            $.post("<?php echo U('Jingli/statuss');?>",PostData,
                function(data){
                });
  ...
  // 修改普通管理员上限人数
        $('.user_num input').blur(function(){
           ...
           //发送post请求
            $.post("<?php echo U('Jingli/numberofpeople');?>",PostData,
                function(data){
                });
            // console.log(PostData);
